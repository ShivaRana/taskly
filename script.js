...
const handleOnSubmit = (e) => {
  ...
  const hr = +newForm.get('hr'); //+ converts from string to number
  const taskObj = {
    task,
    hr,
  ...
  //check for available hours to be added
  const currentTotalHrs = taskTotal();
  if (currentTotalHrs + hr > hoursPerWeek) {
    return alert(
      "Sorry, you can't add this task because it exceeds the maximum limit which is 168hrs.",
    );
  }
...
const displayBadList = () => {
  ...
  document.getElementById('savedHrs').innerText = badList.reduce((acc, list) => acc + list.hr, 0);
};

const randomIdGenerator = (length = 6) => {
  const str =
    'xuyfWpou985321sFCvhmbjdGwuqomczPa8561093wgrQpmnbgdxcuYshja856043';
  let id = '';
...
const taskTotal = () => {
  const totalHrs = taskLists.reduce((acc, list) => {
    return acc + list.hr;
  }, 0);

  document.getElementById('totalHrs').innerText = totalHrs;
  return totalHrs;
};
